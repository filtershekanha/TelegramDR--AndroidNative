...
#define DECODER_FUNC(RETURN_TYPE, NAME, ...) \
  JNIEXPORT RETURN_TYPE Java_com_filtershekanha_teledr_messenger_exoplayer2_ext_opus_OpusDecoder_##NAME(JNIEnv* env, jobject thiz, ##__VA_ARGS__)

#define LIBRARY_FUNC(RETURN_TYPE, NAME, ...) \
  JNIEXPORT RETURN_TYPE Java_com_filtershekanha_teledr_messenger_exoplayer2_ext_opus_OpusLibrary_##NAME(JNIEnv* env, jobject thiz, ##__VA_ARGS__)
...
DECODER_FUNC(jlong, opusInit, jint sampleRate, jint channelCount,
  ...
  const jclass outputBufferClass = env->FindClass(
      "com/filtershekanha/teledr/messenger/exoplayer2/decoder/SimpleOutputBuffer");
