...
import com.filtershekanha.teledr.VersionCheckService;
...
import com.filtershekanha.teledr.internal.BuildVars;
import com.filtershekanha.teledr.messenger.ChatObject;
import com.filtershekanha.teledr.messenger.ContactsController;
import com.filtershekanha.teledr.messenger.DataQuery;
import com.filtershekanha.teledr.messenger.DialogObject;
import com.filtershekanha.teledr.messenger.FileLog;
import com.filtershekanha.teledr.messenger.ImageLoader;
import com.filtershekanha.teledr.messenger.LocaleController;
import com.filtershekanha.teledr.messenger.MessageObject;
import com.filtershekanha.teledr.messenger.MessagesController;
import com.filtershekanha.teledr.messenger.MessagesStorage;
import com.filtershekanha.teledr.messenger.NotificationCenter;
import com.filtershekanha.teledr.messenger.R;
import com.filtershekanha.teledr.messenger.SharedConfig;
...
import com.filtershekanha.teledr.messenger.support.widget.LinearSmoothScrollerMiddle;
...
import com.filtershekanha.teledr.ui.ActionBar.BottomSheet;
import com.filtershekanha.teledr.ui.ActionBar.MenuDrawable;
import com.filtershekanha.teledr.ui.ActionBar.Theme;
import com.filtershekanha.teledr.ui.ActionBar.ThemeDescription;
import com.filtershekanha.teledr.ui.Adapters.DialogsAdapter;
import com.filtershekanha.teledr.ui.Adapters.DialogsSearchAdapter;
import com.filtershekanha.teledr.ui.Cells.AccountSelectCell;
import com.filtershekanha.teledr.ui.Cells.DialogCell;
import com.filtershekanha.teledr.ui.Cells.DialogsEmptyCell;
import com.filtershekanha.teledr.ui.Cells.DividerCell;
import com.filtershekanha.teledr.ui.Cells.DrawerActionCell;
import com.filtershekanha.teledr.ui.Cells.DrawerAddCell;
import com.filtershekanha.teledr.ui.Cells.DrawerProfileCell;
import com.filtershekanha.teledr.ui.Cells.DrawerUserCell;
import com.filtershekanha.teledr.ui.Cells.GraySectionCell;
import com.filtershekanha.teledr.ui.Cells.HashtagSearchCell;
import com.filtershekanha.teledr.ui.Cells.HintDialogCell;
import com.filtershekanha.teledr.ui.Cells.LoadingCell;
import com.filtershekanha.teledr.ui.Cells.ProfileSearchCell;
import com.filtershekanha.teledr.ui.Cells.UserCell;
import com.filtershekanha.teledr.ui.Components.AlertsCreator;
import com.filtershekanha.teledr.ui.Components.AnimatedArrowDrawable;
...
import com.filtershekanha.teledr.ui.Components.ChatActivityEnterView;
import com.filtershekanha.teledr.ui.Components.CombinedDrawable;
import com.filtershekanha.teledr.ui.Components.EmptyTextProgressView;
import com.filtershekanha.teledr.ui.Components.FragmentContextView;
import com.filtershekanha.teledr.ui.Components.JoinGroupAlert;
import com.filtershekanha.teledr.ui.Components.LayoutHelper;
import com.filtershekanha.teledr.ui.Components.ProxyDrawable;
...
import com.filtershekanha.teledr.ui.Components.SizeNotifierFrameLayout;
import com.filtershekanha.teledr.ui.Components.StickersAlert;

import java.util.ArrayList;
import java.util.concurrent.TimeUnit;

import static com.filtershekanha.teledr.VersionCheckService.ACTION_UPGRADE_NEEDED;
import static com.filtershekanha.teledr.VersionCheckService.EXTRA_APK_URL;
import static com.filtershekanha.teledr.VersionCheckService.EXTRA_PLAY_URL;
import static com.filtershekanha.teledr.VersionCheckService.EXTRA_REQUIRED;
...
public class DialogsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void onFragmentDestroy() {
        ...
        if (versionCheckReceiver != null) {
            LocalBroadcastManager.getInstance(ApplicationLoader.applicationContext).unregisterReceiver(versionCheckReceiver);
        }
    ...
    private void setupVersionCheck() {
        ...
        versionCheckReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (ACTION_UPGRADE_NEEDED.equals(intent.getAction())) {
                    showUpgradeDialog(
                            intent.getBooleanExtra(EXTRA_REQUIRED, false),
                            intent.getStringExtra(EXTRA_PLAY_URL),
                            intent.getStringExtra(EXTRA_APK_URL));
                }
            }
        };
        IntentFilter filter = new IntentFilter(ACTION_UPGRADE_NEEDED);
        LocalBroadcastManager.getInstance(ApplicationLoader.applicationContext).registerReceiver(versionCheckReceiver, filter);
    }

    private void showUpgradeDialog(final boolean required, final String playUrl, final String apkUrl) {
        final Activity parentActivity = getParentActivity();

        if (parentActivity == null || parentActivity.isFinishing()) return;

        AlertDialog.Builder builder = new AlertDialog.Builder(parentActivity)
                .setTitle(parentActivity.getString(R.string.upgrade_title))
                .setMessage(parentActivity.getString(R.string.upgrade_msg));

        if (required) {
            builder.setNeutralButton(parentActivity.getString(R.string.upgrade_quit), new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialogInterface, int i) {
                    preferences.edit().remove("version_check_timestamp").apply();
                    parentActivity.finish();
                }
            });
        } else {
            builder.setNeutralButton(parentActivity.getString(R.string.upgrade_cancel), new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialogInterface, int i) {
        ...
        builder.setPositiveButton(parentActivity.getString(R.string.upgrade_play), new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                if (required) preferences.edit().remove("version_check_timestamp").apply();
                openPlayLink(playUrl);
            }
        });
//        builder.setNegativeButton(parentActivity.getString(R.string.upgrade_apk), new DialogInterface.OnClickListener() {
//            @Override
//            public void onClick(DialogInterface dialogInterface, int i) {
//                if (required) preferences.edit().remove("version_check_timestamp").apply();
//                openApkLink(apkUrl);
...
//        });
        ...
        if (upgradeDialog == null || !upgradeDialog.isShowing()) {
            ...
            upgradeDialog.setCancelable(!required);
            upgradeDialog.show();
        }
    }

    private void openPlayLink(String linkUrl) {
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setData(Uri.parse(linkUrl));
        getParentActivity().startActivity(intent);
    }

    private void openApkLink(String linkUrl) {
        downloadReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                Log.d("VersionCheck", "APK download completed.");
            }
        };
        ApplicationLoader.applicationContext.registerReceiver(downloadReceiver, new IntentFilter(DownloadManager.ACTION_DOWNLOAD_COMPLETE));

        DownloadManager.Request request = new DownloadManager.Request(Uri.parse(linkUrl));
        request.setTitle("TeleDR");
        request.setDescription("Downloading TeleDR...");
        request.allowScanningByMediaScanner();
        request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
        request.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS, "telegram-dr.apk");

        DownloadManager manager = (DownloadManager) ApplicationLoader.applicationContext.getSystemService(Context.DOWNLOAD_SERVICE);
        if (manager != null) {
            manager.enqueue(request);
        } else {
            Log.e("VersionCheck", "Manager is null");
        }
    }

    private void checkForAppUpgrade() {
        Log.d("VersionCheck", "Checking for App new version...");
        if (isVersionCheckNeeded() && ConnectionsManager.isNetworkOnline()) {
            Context context = ApplicationLoader.applicationContext;
            context.startService(new Intent(context, VersionCheckService.class));
        } else {
            Log.d("VersionCheck", "Aborting... Check not needed yet");
        }
    }

    @SuppressWarnings("ConstantConditions")
    private boolean isVersionCheckNeeded() {
        long checkTimestamp = preferences.getLong("version_check_timestamp", 0);
        long delay = BuildConfig.DEBUG ? TimeUnit.MINUTES.toMillis(1) : TimeUnit.HOURS.toMillis(6);
        return System.currentTimeMillis() - checkTimestamp > delay;
    }
