...
import com.filtershekanha.teledr.tgnet.SerializedData;
...
public class SharedConfig {
    ...
    public static class ProxyInfo {
        ...
        public static ProxyInfo fromJson(String json) {
            try {
                ...
                return new ProxyInfo(server, port, null, null, secret);
            } catch (JSONException e) {
                return new ProxyInfo(null, -1, null, null, null);
            }
        }
    }

    public static ArrayList<ProxyInfo> proxyList = new ArrayList<>();
    public static ArrayList<ProxyInfo> proxyBlockedList = new ArrayList<>();
    private static boolean proxyListLoaded;
    private static boolean proxyBlockedListLoaded;
    ...
    public static void saveProxyList() {
        Log.d("TeleDR", "Saving proxy list. Total adresses: " + proxyList.size());
    ...
    public static void saveBlockedProxyList() {
        Log.d("TeleDR", "Saving blocked proxy list. Total blocked adresses: " + proxyBlockedList.size());

        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
        //Clear blocked adresses if there are too many
        if (proxyBlockedList.size() > 500) {
            preferences.edit().remove("proxy_list_blocked").commit();
        }

        SerializedData serializedData = new SerializedData();
        int count = proxyBlockedList.size();
        serializedData.writeInt32(count);
        for (int a = 0; a < count; a++) {
            ProxyInfo info = proxyBlockedList.get(a);
            serializedData.writeString(info.address != null ? info.address : "");
            serializedData.writeInt32(info.port);
            serializedData.writeString(info.username != null ? info.username : "");
            serializedData.writeString(info.password != null ? info.password : "");
        ...
        preferences.edit().putString("proxy_list_blocked", Base64.encodeToString(serializedData.toByteArray(), Base64.NO_WRAP)).commit();
        serializedData.cleanup();
    }

    public static void loadBlockedProxyList() {
        if (proxyBlockedListLoaded) {
            return;
        }
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);

        proxyBlockedListLoaded = true;
        proxyBlockedList.clear();
        String list = preferences.getString("proxy_list_blocked", null);
        if (!TextUtils.isEmpty(list)) {
            byte[] bytes = Base64.decode(list, Base64.DEFAULT);
            SerializedData data = new SerializedData(bytes);
            int count = data.readInt32(false);
            for (int a = 0; a < count; a++) {
                ...
                proxyBlockedList.add(info);
            }
            data.cleanup();
        }
        Log.d("TeleDR", "Loading blocked proxy list. Total blocked adresses: " + proxyBlockedList.size());
    }
...
//            editor.putBoolean("proxy_enabled_calls", false);
    ...
    public static void deleteAllProxies() {
        ...
        saveProxyList();
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
        preferences.edit().remove("proxy_list").commit();
        ConnectionsManager.setProxySettings(false, "", 0, "", "", "");
    }

    public static boolean isBlocked(ProxyInfo proxyInfo) {
        for (ProxyInfo info : proxyBlockedList) {
            if (info.secret.equals(proxyInfo.secret) && info.address.equals(proxyInfo.address)) {
                return true;
            }
        }
        return false;
